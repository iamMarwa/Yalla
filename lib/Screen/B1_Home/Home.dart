...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    ...
    return KeysToBeInherited(
      ...
      child: Scaffold(
        ...
        separatorBuilder: (BuildContext context,int index){return SizedBox(height:MediaQuery.of(context).size.height*0.04);},
 ...
 Navigator.push(context,   MaterialPageRoute(builder: (context) =>   newsHeaderListDetail(

                      
                          category:eventDataList[index].category,
                          desc: eventDataList[index].description,
                          price: eventDataList[index].price,
                          imageUrl: eventDataList[index].imageUrl,
                          //index: list[i].reference,
                          time: eventDataList[index].hours,
                          date: eventDataList[index].date,
                          place: eventDataList[index].location,
                          title: eventDataList[index].title,
                          id: eventDataList[index].id,
                          userId: widget.userId,
                          desc2: eventDataList[index].description2,
                          desc3: eventDataList[index].description3,
                          host_name: eventDataList[index].host_name,
                          type: eventDataList[index].type,
                  ...
                  //     );
                   //}
                        ...
                        child: 
                      cardDataFirestore(widget.userId,eventDataList[index])
                      ,)
                      ):
                    Text('sdfas');
                  }
...
async {
  ...
  {
    await getDistance (tmp[i].positiononmap.latitude,tmp[i].positiononmap.longitude).then((value) {
...
Future<double> getDistance (double lat,double long) async {
   ...
   double distanceInMeters = await Geolocator.distanceBetween(lat, long, currentpositon_geo.longitude,currentpositon_geo.longitude);
